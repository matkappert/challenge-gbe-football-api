 ...
 *
 * v0.9.4
 * - added support for lodash 4.x (@matkappert)
 *
 * v0.9.3
 *
...
(function(factory, exposeUMD){
  ...
  function _wrapMethodsAndVerifyNoArrowFunctions(def, currentModuleEntityNoun){
    ...
    _.each(def, function (x, propertyName) {
      if (x !== undefined) {
        if (_.includes(_.keys(RECOMMENDATIONS_BY_UNRECOGNIZED_KEY), propertyName) || _.includes(_.keys(lowercasedRecommendationsByKey), propertyName.toLowerCase())) {
  ...
  // ---------------------------------------------------------------------------------------
  ///////////////////////////////////////////////////////////////////////////////////////////
  if (false && $ && typeof window !== 'undefined' && window.SAILS_LOCALS && window.SAILS_LOCALS._environment !== 'production') {
        ...
        $('<div id="parasails-error-handler">'+
          '<div role="error-handler-content">'+
            ...
            '<p>'+
              ...
              '<pre>'+_.escape(_.truncate(errorSummary, {length: 350}))+'</pre>'+
  ...
  parasails.registerPage = function(pageName, def){
    ...
    if (typeof window === 'undefined' || !window.SAILS_LOCALS || window.SAILS_LOCALS._environment !== 'development') {
      def.devtools = false;
      def.debug = false;
      def.silent = true;
